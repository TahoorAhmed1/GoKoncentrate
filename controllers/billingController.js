  ...
  index:async function(req,res){
    try{
      var get_all_billing_requests= await billing.findAll({
        attributes:['id','magazineBrandId','userId','status',[sequelize.literal(`(SELECT name FROM users WHERE id = billing.user_id)`), 'userName'],[sequelize.literal(`(SELECT name FROM magazines_brand WHERE id = billing.magazine_brand_id)`), 'magazineBrandName']],
       order:[
         ['id','desc']
       ],
        raw:true
